$(document).ready(function () {
    ...
    $(document).on('click', '#searchImg', function () {
        var width = $("#searchTextBox").width();

        //New Code added to handle the new failSearchBox
        if (width < 1)
        {
            $("#panMode").click();
            $("#searchTextBox").css({ display: "inline-block" });
            $("#searchTextBox").animate({ width: 150 }, 150);
            $("#searchTextBox").focus();
        }
        else {
            $("#searchImg").css({ backgroundColor: "transparent" });
            $("#searchTextBox").animate({ width: 0 }, 150);
            $("#searchTextBox").delay(500).css({ display: "none" });
            $("#failSearchBox").css({ visibility: "hidden" });
            $("#searchTextBox").val("");
        }
    });

        $(document).on('keypress', '#searchTextBox', function (e) {
            if (e.which == 13)
            {
        ...
        {
            var gg = boardContent.multiWordTexts;
            var e;
            var counter = 0;
            var bb = [];
            var innerText = "";
            ...
            for (var key in gg)
            {
                try
                {
                    ...
                    for (var holder in bb)
                    {
                        innerText += bb[holder].text.plainText + " ";
                    }

                    if (innerText.toUpperCase().indexOf(criteria.toUpperCase()) > -1) {
                        counter++;
                        e = gg[key];
                        zoomToFound(e);
                        $("#panMode").click();
            ...
            //New Code added to handle the new failSearchBox
            if (counter == 0) {
                $("#failSearchBox").css({ visibility: "visible" });
            }
            else
            {
                $("#failSearchBox").css({ visibility: "hidden" });
        ...
        function zoomToFound(a)
        {
            Progress.onBoardContentChanged.autoZooming = zoomToFit;
            takeControlOfViewbox(!1);
            var tentX = a.x - (a.width * .8);

            if ("text" != Modes.currentMode.name)
            {
                var b = scaleScreenToWorld($("#masterHeader .heading").height()),
                    e = Modes.select.handlesAtZoom();
                requestedViewboxWidth = boardContent.width + 2 * e;
                requestedViewboxHeight = boardContent.height + b + 2 * e;
                IncludeView.specific(tentX, a.y - (b + e / 2), a.width * 2, a.height * 2, a)
            }
        }
});
